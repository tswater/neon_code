...
core_var = ['TSS_FILTER','TSS_FILTER_HP','T_SONIC_SIGMA','H','BLH','VEG','TREE',\
...
opt_EBpct_min = 0
opt_EBpct      = 1    # Energy balance closure residual max
...
for file in sites:
    ...
    if (opt_EBdaily):
        ...
        if 'EB_RES' not in var_stor.keys():
            var_stor['EB_RES']=[]
    ...
    for p in core_var:
        ...
        if (p=='TSS_FILTER') or (p=='HSS_FILTER'):
            mask=mask&(fp_in[p][:]>=0)
        if (p=='TSS_FILTER_HP'):
            mask=mask&~np.isnan(fp_in[p])&(fp_in[p]>=0)
    ...
    # ADD EB RESIDUAL
    var_stor['EB_RES'].extend(res[mask]/(fp_in['NETRAD'][mask]+.00034))
