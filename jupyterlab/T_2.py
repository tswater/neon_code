...
# %%
plt.figure()
plt.hexbin(zL,ustar**2/H**2*o_Tsig**2,gridsize=300,cmap='terrain',mincnt=1,extent=(-2.5,1.5,0,8))
zL2=np.linspace(-2.5,1.5,50)
a = 4
b = 8.3
...
plt.figure()
zL_x=np.log(-zL)
y_ =ustar**2/H**2*o_Tsig**2
m_nan=(~np.isnan(zL_x))&(~np.isnan(y_))
plt.hexbin(zL_x[m_nan],y_[m_nan],gridsize=300,cmap='terrain',mincnt=1,extent=(-7,4.5,0,14))
zL2=-np.logspace(-7,4.5)
a = 4
b = 8.3
andre_perfect=a*(1-b*zL2)**(-.66)
andre_perfect[zL2>0]=a
plt.plot(np.log(-zL2),andre_perfect,'r--')
plt.xlabel('log(-ZL): Monin-Obukhov Stability Param',fontsize=14)
plt.ylabel('Andre LHS (non-dimensional)',fontsize=14)
plt.xlim(-7,4.5)
plt.ylim(0,14)
...
plt.figure()
zL_x=np.log(-zL)
y_ =np.log(ustar**2/H**2*o_Tsig**2)
m_nan=(~np.isnan(zL_x))&(~np.isnan(y_))
plt.hexbin(zL_x[m_nan],y_[m_nan],gridsize=300,cmap='terrain',mincnt=1,extent=(-7,4.5,-5,5))
zL2=np.linspace(-200,-0.001,100)
a = 7.9
b = 28.3
andre_perfect=a*(1-b*zL2)**(-.66)
andre_perfect[zL2>0]=a
plt.plot(np.log(-zL2),np.log(andre_perfect),'r--')
plt.xlabel('ZL: Monin-Obukhov Stability Param',fontsize=14)
plt.ylabel('Andre LHS (non-dimensional)',fontsize=14)
plt.xlim(-7,4.5)
...
for site in sites:
    ...
    m=m_site
    plt.hexbin(,gridsize=50,cmap='terrain',mincnt=1,extent=[0,1.25,0,1.25])
...
for rect in bar1:
    ...
    plt.text(rect.get_x() + rect.get_width()/2.0, .91,str(int(round(errs[i]*100))), ha='center', va='bottom',color='red')
