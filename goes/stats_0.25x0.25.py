...
for j in grid['pos_y'][:-1]:
    ...
    for i in grid['pos_x'][:-1]:
        ...
        if not (pos_x==pos_y):
            ...
            continue
        print(grid['site'][pos_y])
        data=lst[int(j):int(j+10),int(i):int(i+10)]
                
        if (np.sum(data<=0)/(10*10)>lim):
            cl_5[int(pos_y),int(pos_x)]=np.nan
            cl_10[int(pos_y),int(pos_x)]=np.nan
            cl_15[int(pos_y),int(pos_x)]=np.nan
            cl_20[int(pos_y),int(pos_x)]=np.nan
            ...
            P50[int(pos_y),int(pos_x)]=np.nan
            P75[int(pos_y),int(pos_x)]=np.nan
            flag[int(pos_y),int(pos_x)]=1-(np.sum(data<=0)/(10*10)) #1 all cells have data; 0 no cell has data
        else:
            xs=np.linspace(min_lon[int(pos_y),int(pos_x)],max_lon[int(pos_y),int(pos_x)],11) #Limits longitudes
            XC=np.zeros([10,10]) #Initialize cental longitudes for 1arcdegree boxes
            ys=np.linspace(min_lat[int(pos_y),int(pos_x)],max_lat[int(pos_y),int(pos_x)],11) #Limits latitudes
            YC=np.zeros([10,10]) #Initialize cental latitudes for 1arcdegree boxes
            ...
            if (abs(cen_lon[int(pos_y),int(pos_x)]-mean_X)>lim_c) and (abs(cen_lat[int(pos_y),int(pos_x)]-mean_Y)>lim_c):
                ...
                flag[int(pos_y),int(pos_x)]=1-(np.sum(data==-9999)/(10*10))
            else:
