...
core_var = ['T_SONIC_SIGMA','LST_1_STD','H','H2O','USTAR','T_SONIC','ZL','WS']
...
for file in sites:
    ...
    netrad_sum = 0
    jlist = []
    reses = np.ones((n,))*-9999
    resesp=np.ones((n,))*-9999
    for j in range(n):
        if not mask[j]:
            continue
    ...
    if 'EBD' not in var_stor.keys():
        var_stor['EBD']=[]
    var_stor['EBD'].extend((reses[mask]))
    if 'EBDp' not in var_stor.keys():
        var_stor['EBDp']=[]
    var_stor['EBDp'].extend((resesp[mask]))
