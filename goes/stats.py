...
tif='/stor/soteria/hydro/shared/data/GOES/GOES-16-EPSG4326/2019/OR_ABI-L2-LSTC-M6_G16_s20191831101248_e20191831104021_c20191831104423.tif'
grid_loc='grid0.25.p'
dx =0.25
...
prs.add_argument('-d', action='store', dest='dx', type=float, default=dx)
prs.add_argument('-i', action='store', dest='tif', default=tif)
prs.add_argument('-g', action='store', dest='grid', default=grid_loc)
prs.add_argument('-o', action='store', dest='out', default=out_dir)
...
# SETUP and SUCH #
# -------------- #
# handle constants for different dx
ndx = round(dx*40)


#SAMPLE SCRIPT TO PARALLEL
#Open grid dictionary
file=open(grid_loc,'rb')
...
timestr=tif.split('_s')[1][0:9]
date=datetime.datetime(int(timestr[0:4]),1,1,0,0)+datetime.timedelta(days=int(timestr[4:7]))+\
     datetime.timedelta(hours=int(timestr[7:9]))
name = out_dir+'cl_'+str(date)+'p'
#name='/home/tsw35/tyche/data/GOES/pickles/0.25x0.25_arcdegree/cl_'+str(date)+'.p'
...
XGRAD=np.zeros([int(grid['pos_y'].shape[0]-1),int(grid['pos_x'].shape[0]-1)])
YGRAD=np.zeros([int(grid['pos_y'].shape[0]-1),int(grid['pos_x'].shape[0]-1)])
XGRADa=np.zeros([int(grid['pos_y'].shape[0]-1),int(grid['pos_x'].shape[0]-1)])
YGRADa=np.zeros([int(grid['pos_y'].shape[0]-1),int(grid['pos_x'].shape[0]-1)])
...
for j in grid['pos_y'][:-1]:
    ...
    for i in grid['pos_x'][:-1]:
        ...
        if not (pos_x==pos_y):
            ...
            XGRAD[int(pos_y),int(pos_x)]=np.nan
            YGRAD[int(pos_y),int(pos_x)]=np.nan
            XGRADa[int(pos_y),int(pos_x)]=np.nan
            YGRADa[int(pos_y),int(pos_x)]=np.nan
            continue
        data=lst[int(j):int(j+ndx),int(i):int(i+ndx)]
                
        if (np.sum(data<=0)/(ndx*ndx)>lim):
            cl_5[int(pos_y),int(pos_x)]=np.nan
            cl_10[int(pos_y),int(pos_x)]=np.nan
            cl_15[int(pos_y),int(pos_x)]=np.nan
            cl_20[int(pos_y),int(pos_x)]=np.nan
            ...
            XGRAD[int(pos_y),int(pos_x)]=np.nan
            YGRAD[int(pos_y),int(pos_x)]=np.nan
            flag[int(pos_y),int(pos_x)]=1-(np.sum(data<=0)/(ndx*ndx)) #1 all cells have data; 0 no cell has data
        else:
            xs=np.linspace(min_lon[int(pos_y),int(pos_x)],max_lon[int(pos_y),int(pos_x)],ndx+1) #Limits longitudes
            XC=np.zeros([ndx,ndx]) #Initialize cental longitudes for 1arcdegree boxes
            ys=np.linspace(min_lat[int(pos_y),int(pos_x)],max_lat[int(pos_y),int(pos_x)],ndx+1) #Limits latitudes
            YC=np.zeros([ndx,ndx]) #Initialize cental latitudes for 1arcdegree boxes
            ...
            for i in range(0,ndx):
                ...
                YC[:,i]=y_cen
            #X and Y Gradients
            dataud=np.flipud(data.copy())
            dataud[dataud==-9999]=np.nan
            grads=np.gradient(dataud)
            ...
            if (abs(cen_lon[int(pos_y),int(pos_x)]-mean_X)>lim_c) and (abs(cen_lat[int(pos_y),int(pos_x)]-mean_Y)>lim_c):
                ...
                XGRAD[int(pos_y),int(pos_x)]=np.nanmean(grads[0])
                YGRAD[int(pos_y),int(pos_x)]=np.nanmean(grads[1])
                XGRADa[int(pos_y),int(pos_x)]=np.nanmean(np.abs(grads[0]))
                YGRADa[int(pos_y),int(pos_x)]=np.nanmean(np.abs(grads[1]))
                flag[int(pos_y),int(pos_x)]=1-(np.sum(data==-9999)/(ndx*ndx))
            else:
...
dict['maps']['x_gradient']=XGRAD
dict['maps']['y_gradient']=YGRAD
dict['maps']['x_gradient_abs']=XGRADa
dict['maps']['y_gradient_abs']=YGRADa
dict['maps']['site']=grid['site']
