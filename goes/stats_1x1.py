...
name='/home/tsw35/tyche/data/GOES/pickles/1x1_arcdegree/cl_'+str(date)+'.p'
...
for j in grid['pos_y'][:-1]:
    ...
    for i in grid['pos_x'][:-1]:
        ...
        data=lst[int(j):int(j+40),int(i):int(i+40)]
        ...
        if (np.sum(data==-9999)/(10*10)>lim) or (0 in data):
            ...
            xs=np.linspace(min_lon[int(pos_y),int(pos_x)],max_lon[int(pos_y),int(pos_x)],41) #Limits longitudes
            XC=np.zeros([40,40]) #Initialize cental longitudes for 1arcdegree boxes
            ys=np.linspace(min_lat[int(pos_y),int(pos_x)],max_lat[int(pos_y),int(pos_x)],41) #Limits latitudes
            YC=np.zeros([40,40]) #Initialize cental latitudes for 1arcdegree boxes
            ...
            if (abs(cen_lon[int(pos_y),int(pos_x)]-mean_X)>lim_c) and (abs(cen_lat[int(pos_y),int(pos_x)]-mean_Y)>lim_c):
                ...
                flag[int(pos_y),int(pos_x)]=1-(np.sum(data==-9999)/(40*40))
            else:
