...
for site in sites[rank::size]:
	...
	# -------------------- #
	try:
		fp_out.create_dataset('TSS_FILTER_HP',data=np.array(out_tssf))
	except:
		fp_out['TSS_FILTER_HP'][:]=np.array(out_tssf)
	fp_out['TSS_FILTER_HP'].attrs['source']='NEON_raw'
	fp_out['TSS_FILTER_HP'].attrs['missing_value']=-9999
	fp_out['TSS_FILTER_HP'].attrs['description']='T_SONIC_SIGMA adjusted for '+\
					  'high pass filter removed trends w/ period <5min'
	fp_out['TSS_FILTER_HP'].attrs['units']='C'
	print('*',flush=True)
